...
from .common import BaseConnectionPool, ConnectionInfo, EndpointPool
...
class ConnectionPool(Generic[EndpointT, ConnectionT], BaseConnectionPool[asyncio.Event, EndpointT, ConnectionT]):
    ...
    async def close(self, graceful_timeout: Optional[float] = None, timeout: Optional[float] = None) -> None:
        ...
        await asyncio.wait_for(self._close_connections(graceful_timeout), timeout=timeout)
    ...
    async def _dispose_connection(self, conn_info: ConnectionInfo[EndpointT, ConnectionT]) -> None:
        try:
            await self._connection_manager.dispose(conn_info.endpoint, conn_info.conn)
    ...
    async def _close_connections(self, graceful_timeout: Optional[float] = None) -> None:
        started_at = time.monotonic()

        def graceful_timeout_exceeded() -> bool:
            return graceful_timeout is not None and time.monotonic() > started_at + graceful_timeout
        ...
        while self._pools:
            ...
            try:
                async with self._lock:
                    ...
                    if acquired and not released and not graceful_timeout_exceeded():
                        with contextlib.suppress(asyncio.TimeoutError):
                            elapsed = time.monotonic() - started_at
                            await asyncio.wait_for(
                                self._lock.wait(),
                                timeout=graceful_timeout - elapsed if graceful_timeout is not None else None,
                            )
